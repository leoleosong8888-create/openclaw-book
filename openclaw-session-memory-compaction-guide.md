...
## 5) 단기 메모리 → 장기 메모리로 넘어가는 상황

단기 세션 메모리는 자동으로 유지되지만, 장기 메모리 파일은 **의미 있는 정보만 선별 저장**하는 것이 좋습니다.

### 5-1) 장기 메모리로 승격해야 하는 신호

다음 유형은 `MEMORY.md` 또는 `memory/*.md`로 옮길 가치가 큽니다.

- 반복 요청되는 사용자 선호
  - 예: 요약 스타일, 알림 빈도, 응답 톤
- 장기 정책/결정
  - 예: 매일 뉴스 배치 유지, 특정 토픽 운영 규칙
- 다음 세션에서도 필요한 작업 컨텍스트
  - 예: 리포지토리 주소, 배포 경로, 운영 체크리스트
- 실패/교훈
  - 예: 자주 발생하는 설정 오류와 해결법

반대로, 아래는 단기 세션에 두는 편이 낫습니다.
- 일회성 질문
- 즉시 소멸되는 상태값(현재 배터리 퍼센트 등)
- 장기 가치가 낮은 잡담
...
세션 대화 누적
  ↓
중요 정보 후보 식별 (선호/결정/장기 작업)
  ↓
요약 문장으로 정리 (중복 제거)
  ↓
MEMORY.md 또는 memory/YYYY-MM-DD.md 기록
  ↓
다음 세션에서 memory_search로 회수
```

### 5-3) 예시

**단기 세션 로그**
- "매일 06:30 미국 뉴스 브리핑 보내줘"
- "변동 없으면 사용량 알림 보내지 마"

**장기 메모리 승격 결과**
- "사용자는 매일 06:30 미국 뉴스 브리핑을 선호"
- "사용량 알림은 임계치 이하/변동 시에만 전송"

핵심은 “원문 대화 전체”를 옮기는 게 아니라, **재사용 가능한 규칙/의사결정**만 옮기는 것입니다.
...
**긴 대화를 운영 가능한 상태로 유지하고, 필요한 정보만 장기 메모리로 승격시키는 최적화 전략**입니다.
